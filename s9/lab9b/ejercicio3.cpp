...
void sum_elements(float** M1, float** M2, float& res, int dim){
    for (int i=0; i < dim; i++)
        for (int j=0; j < dim; j++){ 
            res += M1[i][j] + M2[i][j];
        }
...
void sum_elements_with_threads(float** M1, float** M2, float& res, int dim, int chunk, int chunks){
...
void verificar(float res, int dim){
    assert(res == (float)(dim * dim * 2));
}
...
int main(){
    ...
    //sum_elements(m1,m2,m3,dim);
    for (int i=0; i < n_threads; i++)
        vthreads.push_back(thread(&sum_elements_with_threads,m1,m2,ref(res),dim,i,n_threads));
